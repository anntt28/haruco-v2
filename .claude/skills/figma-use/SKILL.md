...
18. **For FigSpec (`figma.spec.*`) calls, pass markdown as normal JS strings.** Do not shell/base64-wrap spec content or decode it with `atob()` inside the script. `content`, `oldText`, and `newText` accept direct strings; use `editContent` for non-empty specs and reserve base64 data URLs for actual embedded images only.
...
| `"The node with id X does not exist"` | Parent instance was implicitly detached by a child `detachInstance()`, changing IDs | Re-discover nodes by traversal from a stable (non-instance) parent |
...
- [ ] FigSpec markdown is passed directly to `figma.spec.*` as JS strings — no base64 transport except actual image data URLs
